...
public class Calibrador {
    List<String> calibrador = new ArrayList<>(Arrays.asList("ckmb52fldxkseven3fkjgcbzmnr7",
    ...
    public List<String> calibrarNumerosELetras(Map<String, String> mapNumerosSozinhos, Map<String,
            String> mapNumerosCombinados, List<String> numerosSozinhos, List<String> numerosCombinados){
        for(int i = 0; i < this.calibrador.size(); i ++){
            String resultado = this.calibrador.get(i);
            for (int j = 0; j < numerosCombinados.size(); j++){
                if (this.calibrador.get(i).contains(numerosCombinados.get(j))){
                    resultado = resultado.replace(numerosCombinados.get(j), mapNumerosCombinados.get(numerosCombinados.get(j)));
                }
            }
            for (int h = 0; h < numerosSozinhos.size(); h++){
                if (this.calibrador.get(i).contains(numerosSozinhos.get(h))){
                    resultado = resultado.replace(numerosSozinhos.get(h), mapNumerosSozinhos.get(numerosSozinhos.get(h)));
                }
            }
            System.out.println("Texto original: " + this.calibrador.get(i));
            System.out.println("Texto substituído: " + resultado);
            this.calibrador.set(i, resultado);
        }
        return this.calibrador;
    }

    public int calibrarSoNumeros(){
        for(int i = 0; i < this.calibrador.size(); i++){
            String linha = this.calibrador.get(i);
            int contador1 = 0;
            while (contador1 < linha.length()){
                char caracteri = linha.charAt(contador1);
                if (Character.isDigit(caracteri)) {
                    numero1 = String.valueOf(caracteri);
                    break;
                }
                contador1 += 1;
            }
            ...
            while (contador2 >= 0) {
                char caracteri = linha.charAt(contador2);
                if (Character.isDigit(caracteri)) {
                    numero2 = String.valueOf(caracteri);
                    break;
                }
                contador2 -= 1;
            }
            String concatenacao = numero1 + numero2;
            this.calibrador.set(i, concatenacao);
            int concatenacaoInteiro = Integer.parseInt(concatenacao);
            somaTotal += concatenacaoInteiro;
        }
